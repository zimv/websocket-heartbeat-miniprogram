class WebsocketHeartbeatJs {
  constructor({
    ...
    if (!miniprogram) {
      const err = Error(
        '请传入小程序全局实例：Taro:Taro,支付宝my,百度swan，微信wx等'
      );
      reject(err);
      throw err;
    }
  ...
  async createWebSocket(resolve) {
    ...
    if (this.opts.miniprogram.offSocketClose) {
      ...
      this.opts.miniprogram.connectSocket(this.opts.connectSocketParams);
    } else {
      //wx,Taro,百度等。Taro返回的promise，所以这里使用await，兼容promise和直接返回对象
      this.socketTask = await this.opts.miniprogram.connectSocket(
        this.opts.connectSocketParams
      );
    }
    //本打算直接把socketTask的on相关方法直接赋值给
...
export default param => {
  return new Promise((resolve, reject) => {
    new WebsocketHeartbeatJs(param, {
      resolve,
      reject
    });
  });
